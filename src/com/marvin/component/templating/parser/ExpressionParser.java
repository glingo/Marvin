...
import com.marvin.component.templating.node.FunctionOrMacroNameNode;
...
import com.marvin.component.templating.node.expression.ArrayExpression;
import com.marvin.component.templating.node.expression.BinaryExpression;
import com.marvin.component.templating.node.expression.BlockFunctionExpression;
...
import com.marvin.component.templating.node.expression.LiteralBooleanExpression;
import com.marvin.component.templating.node.expression.LiteralDoubleExpression;
import com.marvin.component.templating.node.expression.LiteralLongExpression;
import com.marvin.component.templating.node.expression.LiteralNullExpression;
import com.marvin.component.templating.node.expression.LiteralStringExpression;
...
public class ExpressionParser {
    ...
    private Expression<?> parseExpression(int minPrecedence) throws Exception {
        ...
        if (isUnary(token)) {
            ...
            Class<? extends Expression<?>> operatorNodeClass = operator.getNodeClass();
            try {
                unaryExpression = (UnaryExpression) operatorNodeClass.newInstance();
        ...
        while (isBinary(token) && binaryOperators.get(token.getValue()).getPrecedence() >= minPrecedence) {
            ...
            Class<? extends Expression<?>> operatorNodeClass = operator.getNodeClass();
            try {
                finalExpression = (BinaryExpression<?>) operatorNodeClass.newInstance();
                finalExpression.setLineNumber(stream.current().getLineNumber());
            } catch (InstantiationException | IllegalAccessException e) {
//                e.printStackTrace();
                String msg = String.format("Error instantiating operator node [%s] at line %s in file %s.", operatorNodeClass.getName(), token.getLineNumber(), stream.getFilename());
